...
Viam module that registers the components and the scene service of an automation workcell. As of 0.7.0 there are fifteen registered models (thirteen visual/affordance models, plus two sensors):
...
- **Frame origin is not the same on every model** — see the "Frame origins" table in README.md. Centroid-anchored: `pallet`, `pick-station`, `workcell-bounds` (for these, set `frame.translation.z = thickness/2` or `height/2` so the bottom rests on the floor). Floor-anchored (z=0 puts the base on the floor): `safety-fence`, `light-curtain`, `e-stop`, `stack-light`, `robot-pedestal`, `hmi-cabinet`, `floor-decal`. Stack-from-base: `tote-stack` (the bottom box's bottom face is at the frame origin). When adding a new model, document its origin in the README table, the source-file header doc-comment, and the model's `description` in `meta.json`.
- **Internal pose: pallet keeps its centroid; pick-station composes a corner offset internally** so pack-order math reads from the bottom-left-top corner outward. Both expose `get_visual_pose` returning the centroid (what the scene service needs).
...
└── cmd/module/main.go         (registers all 15 models via module.ModularMain)
```
